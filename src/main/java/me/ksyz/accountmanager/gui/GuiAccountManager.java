...
public class GuiAccountManager extends GuiScreen {
    ...
    public void initGui() {
        ...
        buttonList.add(new GuiButton(
                1, width / 2 - 105, height - 52, 90, 20, "Microsoft"
        ));
        buttonList.add(new GuiButton(
                6, width / 2 - 10, height - 52, 90, 20, "Cracked"
        ));
        buttonList.add(new GuiButton(
                4, width / 2 + 85, height - 52, 90, 20, "Session"
        ));
    ...
    protected void actionPerformed(GuiButton button) {
        ...
        if (button.enabled) {
            switch (button.id) {
                ...
                break;
                case 6: {
                    mc.displayGuiScreen(new GuiCrackedAccount(previousScreen));
                }
                break;
                default: {
    ...
    private void loginCracked(Account account) {
        String username = account.getUsername();
        if (StringUtils.isBlank(username)) {
            notification = new Notification(TextFormatting.translate("&cInvalid cracked username.&r"), 5000L);
            return;
        }

        String trimmedUsername = username.trim();
        SessionManager.set(new Session(trimmedUsername, "", "", "legacy"));
        notification = new Notification(TextFormatting.translate(String.format(
                "&aSuccessful cracked login! (%s)&r", trimmedUsername
        )), 5000L);
    }
    ...
    class GuiAccountList extends GuiSlot {
        ...
        protected void drawSlot(int entryID, int x, int y, int k, int mouseXIn, int mouseYIn) {
            ...
            if (StringUtils.isBlank(username)) {
                username = "&7&l?";
            } else if (!account.isCracked() && account.getAccessToken().equals(SessionManager.get().getToken())) {
            ...
            String unban;
            if (account.isCracked()) {
                unban = account.getUsername().equals(SessionManager.get().getUsername()) ? "&aCracked" : "&7Cracked";
            } else {
