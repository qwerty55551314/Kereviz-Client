...
public abstract class MixinEntityRenderer {
    ...
    private float hurtCameraEffect(float float1) {
        if (Kereviz.moduleManager == null) {
            return float1;
        } else {
            NoHurtCam noHurtCam = (NoHurtCam) Kereviz.moduleManager.modules.get(NoHurtCam.class);
    ...
    private void a(
        ...
        if (Kereviz.moduleManager != null) {
            GhostHand event = (GhostHand) Kereviz.moduleManager.modules.get(GhostHand.class);
            if (event.isEnabled()) {
    ...
    private Material x(Block block) {
        if (Kereviz.moduleManager == null) {
            ...
            return Kereviz.moduleManager.modules.get(ViewClip.class).isEnabled() ? Material.air : block.getMaterial();
        }
    }
    ...
    private boolean y(EntityLivingBase entityLivingBase, Potion potion) {
        if (potion == Potion.blindness && Kereviz.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) Kereviz.moduleManager.modules.get(AntiDebuff.class);
    ...
    private boolean c(EntityPlayerSP entityPlayerSP, Potion potion) {
        if (potion == Potion.confusion && Kereviz.moduleManager != null) {
            AntiDebuff antiDebuff = (AntiDebuff) Kereviz.moduleManager.modules.get(AntiDebuff.class);
