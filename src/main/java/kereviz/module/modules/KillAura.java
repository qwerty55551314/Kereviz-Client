...
import kereviz.management.RotationState;
import kereviz.mixin.IAccessorPlayerControllerMP;
import kereviz.mixin.IAccessorRenderManager;
...
public class KillAura extends Module {
    ...
    private boolean performAttack(float yaw, float pitch) {
        if (!Kereviz.playerStateManager.digging && !Kereviz.playerStateManager.placing) {
    ...
    private boolean canAttack() {
        if (this.inventoryCheck.getValue() && mc.currentScreen instanceof GuiContainer) {
            ...
            if (((IAccessorPlayerControllerMP) mc.playerController).getIsHittingBlock()) {
                ...
                AutoHeal autoHeal = (AutoHeal) Kereviz.moduleManager.modules.get(AutoHeal.class);
                if (autoHeal.isEnabled() && autoHeal.isSwitching()) {
                    return false;
                } else {
                    BedNuker bedNuker = (BedNuker) Kereviz.moduleManager.modules.get(BedNuker.class);
                    if (bedNuker.isEnabled() && bedNuker.isReady()) {
                        return false;
                    } else if (Kereviz.moduleManager.modules.get(Scaffold.class).isEnabled()) {
                        return false;
    ...
    public void onUpdate(UpdateEvent event) {
        if (event.getType() == EventType.POST && this.blinkReset) {
            ...
            Kereviz.blinkManager.setBlinkState(false, BlinkModules.AUTO_BLOCK);
            Kereviz.blinkManager.setBlinkState(true, BlinkModules.AUTO_BLOCK);
        }
                        ...
                        switch (this.autoBlock.getValue()) {
                            case 0:
                                if (PlayerUtil.isUsingItem()) {
                                    ...
                                    if (!this.isPlayerBlocking() && !Kereviz.playerStateManager.digging && !Kereviz.playerStateManager.placing) {
                                        swap = true;
                                    }
                                } else {
                                    this.isBlocking = false;
                            ...
                            case 2:
                                if (this.hasValidTarget()) {
                                    ...
                                    if (Kereviz.playerStateManager.digging || Kereviz.playerStateManager.placing || mc.thePlayer.inventory.currentItem != item || this.isPlayerBlocking() && this.blockTick != 0 || this.attackDelayMS > 0L && this.attackDelayMS <= 50L) {
                                        this.blockTick = 0;
                                    } else {
                                ...
                                if (this.hasValidTarget()) {
                                    if (!Kereviz.playerStateManager.digging && !Kereviz.playerStateManager.placing) {
                                        switch (this.blockTick) {
                                            ...
                                            case 1:
                                                if (this.isPlayerBlocking()) {
                                                    if (Kereviz.moduleManager.modules.get(NoSlow.class).isEnabled()) {
                            ...
                            case 5:
                                if (this.hasValidTarget()) {
                                    int item = ((IAccessorPlayerControllerMP) mc.playerController).getCurrentPlayerItem();
                                    if (mc.thePlayer.inventory.currentItem == item && !Kereviz.playerStateManager.digging && !Kereviz.playerStateManager.placing) {
                                        switch (this.blockTick) {
                                            case 0:
                                                if (!this.isPlayerBlocking()) {
                                                    swap = true;
                            ...
                            case 8:
                                ...
                                if (PlayerUtil.isUsingItem() && !this.isPlayerBlocking() && !Kereviz.playerStateManager.digging && !Kereviz.playerStateManager.placing) {
                                    swap = true;
                                }
                        }
                    }
                    ...
                    if (this.target != null && this.isBoxInSwingRange(this.target.getBox())) {
                        if (this.rotations.getValue() == 4) {
                            ...
                            if (this.rotations.getValue() == 3) {
                                Kereviz.rotationManager.setRotation(finalYaw, finalPitch, 1, true);
                            } else {
