...
public class CommandManager {
    ...
    public void handleCommand(String string) {
        ...
        if (params.get(0).isEmpty()) {
            ChatUtil.sendFormatted(String.format("%sUnknown command&r", Kereviz.clientName).replace("&", "§"));
        } else {
            ...
            ChatUtil.sendFormatted(String.format("%sUnknown command (&o%s&r)&r", Kereviz.clientName, params.get(0)).replace("&", "§"));
        }
    }
