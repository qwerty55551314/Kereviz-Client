...
import kereviz.enums.DelayModules;
...
public class BedNuker extends Module {
    ...
    private Color getProgressColor(int mode) {
        switch (mode) {
            ...
            case 2:
                return ((HUD) Kereviz.moduleManager.modules.get(HUD.class)).getColor(System.currentTimeMillis());
            default:
                return new Color(-1);
        }
    }
    ...
    public void onTick(TickEvent event) {
        if (event.getType() == EventType.PRE) {
            ...
            if (this.targetBed == null) {
                Kereviz.delayManager.setDelayState(false, DelayModules.BED_NUKER);
            }
        }
    }
    ...
    public void onPlayerUpdate(PlayerUpdateEvent event) {
        if (this.isEnabled()) {
            if (this.isBreaking()
                    && !Kereviz.playerStateManager.attacking
                    && !Kereviz.playerStateManager.digging
                    && !Kereviz.playerStateManager.placing
                    && !Kereviz.playerStateManager.swinging) {
                this.doSwing();
            }
        }
    }
    ...
    public void onRender3D(Render3DEvent event) {
        if (this.isEnabled() && this.targetBed != null && !mc.theWorld.isAirBlock(this.targetBed)) {
            ...
            if (this.showTarget.getValue() != 0) {
                BedESP bedESP = (BedESP) Kereviz.moduleManager.modules.get(BedESP.class);
    ...
    public void onPacket(PacketEvent event) {
        if (!event.isCancelled()) {
            ...
            if (this.isEnabled() && this.targetBed != null && this.ignoreVelocity.getValue() == 2 && Kereviz.delayManager.getDelayModule() != DelayModules.BED_NUKER) {
                if (event.getPacket() instanceof S12PacketEntityVelocity) {
                    S12PacketEntityVelocity packet = (S12PacketEntityVelocity) event.getPacket();
                    if (packet.getEntityID() == mc.thePlayer.getEntityId() && packet.getMotionY() > 0) {
                        Kereviz.delayManager.delay(DelayModules.BED_NUKER);
                        Kereviz.delayManager.delayedPacket.offer(packet);
                        event.setCancelled(true);
                    }
                }
                if (event.getPacket() instanceof S27PacketExplosion) {
                    ...
                    if (explosion.func_149149_c() != 0.0F || explosion.func_149144_d() != 0.0F || explosion.func_149147_e() != 0.0F) {
                        Kereviz.delayManager.delay(DelayModules.BED_NUKER);
                        Kereviz.delayManager.delayedPacket.offer(explosion);
                        event.setCancelled(true);
                    }
                }
            }
