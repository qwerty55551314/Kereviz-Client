...
import kereviz.events.WindowClickEvent;
import kereviz.mixin.IAccessorItemSword;
...
public class ChestStealer extends Module {
    ...
    private boolean isInvManagerRequire(ItemStack itemStack) {
        if (itemStack == null) return false;
        InvManager invManager = (InvManager) Kereviz.moduleManager.modules.get(InvManager.class);
                    ...
                    if (this.oDelay <= 0 && this.clickDelay <= 0) {
                        if (this.isEnabled() && this.isValidGameMode()) {
                            ...
                            if (mc.thePlayer.inventory.getFirstEmptyStack() == -1) {
                                if (!this.warnedFull) {
                                    ChatUtil.sendFormatted(String.format("%s%s: &cYour inventory is full!&r", Kereviz.clientName, this.getName()));
