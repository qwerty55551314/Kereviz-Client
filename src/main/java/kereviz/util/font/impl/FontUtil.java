package kereviz.util.font.impl;
...
public class FontUtil {
    ...
    public static Font getResource(Map<String, Font> locationMap, String location, int size) {
        ...
        try {
            if (locationMap.containsKey(location)) {
                ...
                InputStream is = mc.getResourceManager().getResource(new ResourceLocation("kereviz:font/" + location)).getInputStream();
            ...
            System.err.println("[Kereviz] Failed to load font: " + location);
    ...
    private static void addDevFontFiles(Set<File> files, File start, String location) {
        ...
        for (int i = 0; current != null && i < 8; i++) {
            files.add(new File(current, "src/main/resources/assets/kereviz/font/" + location));
            files.add(new File(current, "assets/kereviz/font/" + location));
            current = current.getParentFile();
        }
    }
}
