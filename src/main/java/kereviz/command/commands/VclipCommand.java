    ...
    public void runCommand(ArrayList<String> args) {
        if (args.size() >= 2) {
            ...
            try {
                ...
                ChatUtil.sendFormatted(String.format("%sClipped (%s blocks)", Kereviz.clientName, df.format(distance)));
            }
            return;
        }
        ChatUtil.sendFormatted(
                String.format("%sUsage: .%s <&odistance&r>&r", Kereviz.clientName, args.get(0).toLowerCase(Locale.ROOT))
        );
    }
}
